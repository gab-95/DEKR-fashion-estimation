...
#    print('heatmap, offset from MODEL-------->',heatmap.shape, offset.shape, image.shape)
    if with_flip:
        ...
        if 'deepfashion2' in cfg.DATASET.ROOT:
            flip_index_heat = FLIP_CONFIG['DEEPFASHION2_WITH_CENTER']
            flip_index_offset = FLIP_CONFIG['DEEPFASHION2']
        if 'deepfashion2_cat1' in cfg.DATASET.ROOT:
            flip_index_heat = FLIP_CONFIG['DEEPFASHION2_cat1_WITH_CENTER']
            flip_index_offset = FLIP_CONFIG['DEEPFASHION2_cat1']

        image = torch.flip(image, [3])
        image[:, :, :, :-3] = image[:, :, :, 3:]
        heatmap_flip, offset_flip = model(image)
#        print('inference get_multi_stage_outputs HEATMAP-->', heatmap.shape)
...
#    print('aggregate_results::h, w, reverse_scale', h,w,reverse_scale)
#    print('aggregate_result::heaatmap shape',heatmap.shape)
