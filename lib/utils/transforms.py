...
def get_multi_scale_size(image, input_size, current_scale, min_scale):
    h, w, _ = image.shape
    center = np.array([int(w / 2.0 + 0.5), int(h / 2.0 + 0.5)])
    #print('utils/transform get_multi_scale_size:: h, w', h, w)
...
#    print('utils/transform.py up_interpolate:: size, x.shape, H, W, scale_h, scale_w-->', size, x.shape, H, W, scale_h, scale_w)
    return padd(inter_x)
